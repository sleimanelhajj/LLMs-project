...
Handles product catalog queries using SQL database with LLM-powered natural language understanding.
The LLM analyzes queries and generates appropriate SQL searches.
"""
...
class CatalogAgent(BaseAgent):
    def __init__(self, db_path: str, google_api_key: str):
        ...
        # Load product catalog for LLM context
    ...
    def _get_catalog_summary(self) -> str:
        """Get a summary of available products for LLM context."""
        try:
    ...
    def _execute_sql(self, query: str, params: tuple = ()) -> List[ProductModel]:
        """Execute SQL query and return results as ProductModel objects."""
        try:
            conn = self._get_connection()
            cursor = conn.cursor()
    ...
    def can_handle(self, query: str) -> bool:
        ...
        keywords = [
            "product", "price", "inventory", "stock", "sku", "catalog",
            "rope", "bag", "wire", "cost", "available", "buy", "purchase"
        ]
        return any(keyword in query.lower() for keyword in keywords)
    
    async def process_query(self, request: QueryRequest) -> AgentResponse:
        """Process catalog query using LLM to understand intent and generate SQL."""
        try:
            query = request.query
            
            # Step 1: Use LLM to understand the query and generate search strategy
            search_strategy = await self._analyze_query_intent(query)
            
            print(f"[CatalogAgent] Search strategy: {search_strategy}")
            
            # Step 2: Execute search based on LLM's strategy
            products = self._execute_search(search_strategy)
            
            # Step 3: Use LLM to format the response
            if products:
...
Respond ONLY with valid JSON, no other text."""
        ...
        try:
            ...
            # Fallback to category search instead of show_all
            return {
    ...
    def _execute_search(self, strategy: Dict[str, Any]) -> List[ProductModel]:
        """Execute SQL search based on LLM's strategy."""
        ...
        # Apply material filter (search in name, description, specifications)
    ...
    async def _generate_response(self, query: str, products: List[ProductModel], strategy: Dict[str, Any]) -> str:
...
{product_data}"""
        ...
        messages = [
            SystemMessage(content=system_prompt.format(
                context=strategy.get("context", "Product search"),
...
# Test function
async def test_catalog_agent():
    """Test the CatalogAgent with LLM-powered search."""
    ...
    print("=" * 80)
    print("CATALOG AGENT TEST - FULLY LLM-POWERED")
    print("=" * 80 + "\n")
    
    agent = CatalogAgent(db_path=CATALOG_DB_PATH, google_api_key=GOOGLE_API_KEY)
    
    test_queries = [
        "What products do you have?",
        "Do you have any rope?",
        "Show me nylon items",
        "I need bags for storage",
        "What's the cheapest rope?",
        "Do you have steel wire?",
        "Show me products under $5",
        "What rope is good for heavy duty use?",
    ]
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        if response.data and response.data.get('products'):
            print(f"\n   📊 Found {len(response.data['products'])} products")
...
if __name__ == "__main__":
    asyncio.run(test_catalog_agent())
