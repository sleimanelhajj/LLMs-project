...
class PolicyAgent(BaseAgent):
    ...
    def _create_system_prompt(self) -> str:
        """Create system prompt for policy Q&A."""
...
Be professional and customer-service oriented."""
    ...
    def can_handle(self, query: str) -> bool:
        ...
        policy_keywords = [
            ...
            "payment", "tax", "discount", "account", "privacy",
            "complaint", "dispute", "terms", "conditions"
        ]
    ...
    async def process_query(self, request: QueryRequest) -> AgentResponse:
        ...
        try:
            ...
            print(f"[PolicyAgent] Searching policies for: {query}")
            ...
            return AgentResponse(
                ...
                data={
                    ...
                    "relevance_scores": [float(chunk['score']) for chunk in retrieved_chunks],  # Convert numpy.float32 to Python float
    ...
    def _format_context(self, chunks: List[Dict[str, Any]]) -> str:
        ...
        for i, chunk in enumerate(chunks, 1):
            source = chunk['metadata'].get('source', 'Policy Document')
            score = chunk['score']
    ...
    async def _generate_answer(self, query: str, context: str, history: List[Dict[str, str]] = None) -> str:
...
async def test_policy_agent():
    """Test PolicyAgent with sample queries."""
    import asyncio
    ...
    vector_db = initialize_policy_vector_db()
    ...
    agent = PolicyAgent(vector_db_manager=vector_db, google_api_key=api_key)
    ...
    for query in test_queries:
        ...
        if response.data:
            print(f"\n📊 Retrieved {response.data.get('retrieved_chunks', 0)} relevant policy sections")
            print(f"   Relevance scores: {response.data.get('relevance_scores', [])}")
